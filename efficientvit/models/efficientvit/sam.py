...
class EfficientViTSam(nn.Module):
    ...
    def __init__(
        ...
        self.mean = torch.tensor([123.675/255, 116.28/255, 103.53/255]).view(1, 3, 1, 1)
        self.std = torch.tensor([58.395/255, 57.12/255, 57.375/255]).view(1, 3, 1, 1)
...
class EfficientViTSamPredictor:
    def __init__(self, sam_model: EfficientViTSam) -> None:
        ...
        self.model.mean = self.model.mean.to(get_device(self.model))
        self.model.std = self.model.std.to(get_device(self.model))
        self.reset_image()
    ...
    def set_image(self, image: np.ndarray|torch.Tensor, image_format: str = "RGB",preprocess=True) -> None:
        ...
        if preprocess:
            torch_data = self.model.transform(image.copy()).unsqueeze(dim=0).to(get_device(self.model))
        else:
            #just do normalization)
            torch_data = (image-self.model.mean)/self.model.std
